    ...
    private IEnumerator EcranDeFin()
    {
        ...
        for (tour = 0; tour < 4; tour++)
        {
            ...
            for (joueur = 0; joueur < 4; joueur++)
            {
                ...
                espace = GetSpawnPoint(joueur, gameManager.playerButton[tour][joueur]);
                ...
                audioSource.clip = textures[gameManager.playerButton[tour][joueur]];
                audioSource.Play();
                yield return new WaitForSeconds(audioSource.clip.length + 0.2f);

                if (joueur != 0)
                {
                    if (gameManager.playerButton[tour][joueur] == gameManager.playerButton[tour][joueur - 1])
                    {
